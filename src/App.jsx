    ...
    <header className={`site-header ${scrolled ? "is-scrolled" : ""}`}>
      ...
      >
        <div className="mobile-menu__inner">
          ...
          <button className="button button--orange button--wide mobile-discovery-trigger" type="button" onPointerEnter={() => window.dispatchEvent(new Event("app:preload-discovery"))} onFocus={() => window.dispatchEvent(new Event("app:preload-discovery"))} onTouchStart={() => window.dispatchEvent(new Event("app:preload-discovery"))} onClick={() => { setOpen(false); window.setTimeout(() => window.dispatchEvent(new CustomEvent("app:open-discovery", { detail: { trigger: menuToggleRef.current, source: menuToggleRef.current } })), 0); }} aria-controls="discovery-dialog"><Search aria-hidden="true" />{language === "tr" ? "Portföyde hızlı ara" : "Quick search the portfolio"}</button>
